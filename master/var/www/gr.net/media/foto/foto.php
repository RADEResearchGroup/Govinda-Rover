...
function sfpg_javascript()
	{
		...
		foreach ($images as $val)
		{
			...
			if($item == 1){
			$java_pic .= '   
				if($(this).attr("id") == "RM-' . sfpg_url_string(GALLERY, $val) . '"){
					if (confirm("' . $text[96] . '")) {
					$("#pesan").show();
					var loadUrl = "/media/foto/foto.php?cmd=del&sfpg=' .  sfpg_url_string(GALLERY, $val) . '";
					$("#pesan").html();
					$.get(
						loadUrl,
			...
			/**print '
				<a href="/media/foto/foto.php?cmd=image&sfpg=' . sfpg_url_string(GALLERY, $val) . '" title="' . str_to_script(@file_get_contents(DATA_ROOT . "info/" . GALLERY . $val . ".sfpg")."|".@file_get_contents(GALLERY_ROOT . GALLERY . $val . DESC_EXT)) . '" data-gallery="">
					<img src="/media/foto/foto.php?cmd=thumb&sfpg=' . sfpg_url_string(GALLERY, $val) .'">
					<p><a href="/media/video/video.php?cmd=file&sfpg=' . sfpg_url_string(GALLERY, $val) . '" title="'. $text[39] .'"> <i class="fa fa-download"></i></a></p>
				</a>
			';**/
			...
			$t_foto .= '
				...
				<div class="thumbnail">
				  <a href="/media/foto/foto.php?cmd=image&sfpg=' . sfpg_url_string(GALLERY, $val) . '" title="' . str_to_script(@file_get_contents(DATA_ROOT . "info/" . GALLERY . $val . ".sfpg")."|".@file_get_contents(GALLERY_ROOT . GALLERY . $val . DESC_EXT)) . '" data-gallery=""><img src="/media/foto/foto.php?cmd=thumb&sfpg=' . sfpg_url_string(GALLERY, $val) . '" title="' . str_to_script(@file_get_contents(DATA_ROOT . "info/" . GALLERY . $val . ".sfpg")."|".@file_get_contents(GALLERY_ROOT . GALLERY . $val . DESC_EXT)) . '" alt="..."></a>
				  <div class="caption">
					<h5>' . str_to_script(sfpg_display_name($val, SHOW_FILE_EXT)) . '</h5>
					<p class="info">' . $text[90] . ' : ' . $date_taken . '</p>
					<p class="info">' . $text[91] . ' : ' . $size . '</p>
					<p class="kontrol"><a href="/media/foto/foto.php?cmd=file&sfpg=' . sfpg_url_string(GALLERY, $val) . '" id="DL-' . sfpg_url_string(GALLERY, $val) . '" title="'. $text[92] .'" title="'. $text[39] .'"> <i class="fa fa-download"></i></a> <a href="#" id="RM-' . sfpg_url_string(GALLERY, $val) . '" title="'. $text[92] .'"><i class="fa fa-trash-o"></i> </a> </p>
				  </div>
				</div>
			  </div>
			';
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (isset($_GET["cmd"]))
	{
		...
		{			
			//Hapus berkas
			//Utama
			unlink($_SERVER['DOCUMENT_ROOT'] . '/media/foto/' . IMAGE);
			//Thumb
			unlink($_SERVER['DOCUMENT_ROOT'] . '/media/foto/_sfpg_data/thumb/' . IMAGE);
			//Info
			unlink($_SERVER['DOCUMENT_ROOT'] . '/media/foto/_sfpg_data/info/' . IMAGE . '.sfpg');
